...
import { aidUrl, disableFooterList } from '@/constants';
import { usePathname } from 'next/navigation';

export default function Footer() {
  const pathname = usePathname();
  const isHidden = new Set(disableFooterList).has(
    `/${pathname.split('?')[0].split('/')[1]}`,
  );

  return (
    <footer
      className={`${isHidden ? 'hidden' : ''} items-between top-0 m-auto flex w-4/5 flex-col space-y-5 py-3 md:mt-16`}
    >
